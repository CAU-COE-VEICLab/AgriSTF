...
class CSModule(nn.Module):
    def __init__(self, in_channels, out_channels, num_heads: int = 32, drop_path_rate=0., choice='CNNSimpleAttention', patch_size = 16):
        super().__init__()
        if choice == 'CNNSimpleAttention':
            self.cnnblock = CNNBlock(in_channels, out_channels)
            self.attention = SimpleAttention(out_channels, num_heads)
        elif choice == 'SimpleAttention':
            self.cnnblock = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=(1, 1))
            self.attention = SimpleAttention(out_channels, num_heads)
...
class CSNet(nn.Module):
    def __init__(self, n_classes, num_heads: int = 2, drop_path_rate=0.0, choice='CNNSimpleAttention', patch_size=16):
        super().__init__()
        ...
        self.down1 = DownCSModule(64, 128, num_heads, dpr[0], choice=choice, patch_size=patch_size)
        ...
        self.up1 = UpConvModule(1024, 256, dpr[0])
...
if __name__ == '__main__':
    ...
    net = CSNet(n_classes=4, num_heads=2, drop_path_rate=0.3, choice='CNNSimpleAttention', patch_size=16).cuda()
    print(net)
