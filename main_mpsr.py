...
def main(config):
    ...
    ema = [0.996, 1.0]
    global momentum_scheduler
    momentum_scheduler = (ema[0] + i*(ema[1]-ema[0])/(len(data_loader_train)*config.TRAIN.EPOCHS)
                        for i in range(int(len(data_loader_train)*config.TRAIN.EPOCHS)+1))
...
def train_one_epoch(config, model, data_loader, optimizer, epoch, lr_scheduler):
    ...
    context_loss_meter = AverageMeter()
    ...
    for idx, (img, mask, _) in enumerate(data_loader):
        ...
        pixel_loss, context_loss = model(img, mask)
        loss = 0.5*pixel_loss + 0.5*context_loss 
        ...
        pixel_loss_meter.update(pixel_loss.item(), img.size(0))
        context_loss_meter.update(context_loss.item(), img.size(0))
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            logger.info(
                ...
                f'pixel loss {pixel_loss_meter.val:.4f} ({pixel_loss_meter.avg:.4f})\t'
                f'context loss {context_loss_meter.val:.4f} ({context_loss_meter.avg:.4f})\t'
